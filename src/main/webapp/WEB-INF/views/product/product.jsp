...
<form action="/auth/product" method="GET" style="display:inline-block">
	<div class="mx-quto input-group m-3">
		...
		<input type="hidden" name="reverseOrder" value="${param.reverseOrder}">
		<input type="hidden" name="category_name" value="${param.category_name}">
		<input type="hidden" name="content_name" value="${param.content_name}">
		<button class="btn btn-info" type="submit" id="button-addon2">검색</button>
	</div>
</form>
<div class="dropdown" style="display:inline-block">
  <button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" id="dropdownBtn">
    	${content_name}
  </button>
  <div class="dropdown-menu">
	...
	<div class="card-group row">
		<c:forEach var="product" items="${products.content}">
			<div id="card" class="card col-3" style="cursor: pointer;" OnClick="location.href ='/product/${product.id}'" data-toggle="tooltip" data-placement="bottom" title="${product.prodBrand}">
				<div style="display: flex; justify-content: center; align-items: center; height: 300px;">
					<img class="card-img-top mt-1" src="${product.filePath}" onerror="this.onerror=null; this.src='https://via.placeholder.com/400.jpg';" style="max-width: 100%; max-height: 100%"
						alt="Card image cap" />
				</div>
				<div class="card-body">
					...
					<p class="card-text"><fmt:formatNumber value="${product.prodKrw}" pattern="#,###" />원
						<c:if test="${product.qty < 3 && product.qty != 0}">
							<a style="color: red; margin-left: 10px;">매진임박</a>
						</c:if>
						<c:if test="${product.qty == 0}">
							<a style="color: red; margin-left: 10px;">매진</a>
						</c:if>
					</p>
					<small class="text-muted mt-3" style="display: block">등록일자: <fmt:formatDate value="${product.createDate}" pattern="yyyy-MM-dd" /></small>
				</div>
			</div>
		</c:forEach>
	</div>
